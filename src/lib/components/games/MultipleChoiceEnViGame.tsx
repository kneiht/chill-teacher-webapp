...
const MultipleChoiceEnViGameCore: React.FC<MultipleChoiceEnViGameProps> = ({
  ...
  const getRandomWrongAnswers = (
    ...
    const wrongOptions = allWords
      .filter((w) => w.vietnameseMeaning !== correctMeaning)
  ...
  const createQuestions = (words: Array<VocabItem>): Array<Question> => {
    const questionList = words.map((word) => {
      ...
      const allOptions = [word.vietnameseMeaning, ...wrongAnswers]
...
const MultipleChoiceEnViGame: React.FC<MultipleChoiceEnViGameActivityProps> = ({
  vocabData,
  backgroundUrl,
  title,
}) => {
  const GameSlide: React.FC<{ isActive: boolean }> = ({ isActive }) => (
    <Slide isActive={isActive}>
      <MultipleChoiceEnViGameCore vocabData={vocabData} title={title} />
    </Slide>
  )
...
export default MultipleChoiceEnViGame
