...
function RouteComponent() {
  ...
  const getItemDetails = (
    ...
    switch (type) {
      case 'video': {
        const video = externalContent?.videos?.find((v: any) => v.id === id)
        if (!video) return null
        return {
          id: `video-${video.id}`,
          title: video.title || 'Video',
          icon: '',
          route: `/lessons/$course/$unit/$lesson/youtube/${video.id}`,
      ...
      case 'googleSlide': {
        ...
        return {
          ...
          icon: '',
          route: `/lessons/$course/$unit/$lesson/googleslide/${slide.id}`,
          description: slide.title,
        }
      }

      case 'embedPage': {
        const embed = externalContent?.embedPages?.find((e: any) => e.id === id)
        if (!embed) return null
        return {
          id: `embed-${embed.id}`,
          title: embed.title || 'Interactive Page',
          icon: '',
          route: `/lessons/$course/$unit/$lesson/embed/${embed.id}`,
      ...
      case 'page': {
        ...
        return {
          ...
          icon: '',
          route: `/lessons/$course/$unit/$lesson/pages/${page.id}`,
      ...
      case 'activity': {
        ...
        return {
          ...
          route: `/lessons/$course/$unit/$lesson/${id}`,
